...
int DES_EncryptBlock(ElemType plainBlock[8], ElemType subKeys[16][48], ElemType cipherBlock[8]){   
    ElemType plainBits[64];   //64bit明文
    ElemType copyRight[48];   //明文右半部分
    int cnt;   
...
int DES_DecryptBlock(ElemType cipherBlock[8], ElemType subKeys[16][48],ElemType plainBlock[8]){   
    ElemType cipherBits[64];   //64bit密文
    ElemType copyRight[48];    //密文右半部分
    int cnt;   
 ...
 */
 //2022150016_huanghaojia
int TripleDES_Encrypt(ElemType *plainBuffer, ElemType *keyBuffer, ElemType *cipherBuffer, int n){
    int des_size = n;
    int des_count = 0;
    int count = 0;
	int i;
 ...
 */
 //2022150016_huanghaojia
int TripleDES_Decrypt(ElemType *cipherBuffer, ElemType *keyBuffer, ElemType *plainBuffer, int n){
    int des_size = n;
    int des_count = 0;
    int count = 0;
	int i;
...
//3DES加密文件
//2022150016_huanghaojia
int TripleDES_Encrypt_File(char *plainFile, char *keyStr,char *cipherFile){   
    ...
    //将文件转为数据流后调用3DES加密数据
	TripleDES_Encrypt(plainBlock,keyBlock,cipherBlock,fileLen_in); 
...
//3DES解密文件
//2022150016_huanghaojia
int TripleDES_Decrypt_File(char *cipherFile, char *keyStr,char *plainFile){   
	FILE *plain,*cipher,*key;
	ElemType *plainBlock,*cipherBlock,keyBlock[24];    
	int count;
	long fileLen_in,fileLen_out;
    ...
    //将文件转为数据流后调用3DES解密数据
	count = TripleDES_Decrypt(cipherBlock,keyBlock,plainBlock,fileLen_in);
